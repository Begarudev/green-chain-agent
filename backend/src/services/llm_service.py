...
def analyze_loan_risk(
    ...
    Args:
        ...
        regulatory_context: Optional formatted regulatory context from RAG service
    ...
    Returns:
        Dictionary containing loan decision and analysis:
        {
            ...
            "compliance_citations": list[str]  # If regulatory_context provided
        }
    """
...
"""
    ...
    # Add regulatory context if provided
    regulatory_section = ""
    if regulatory_context:
        regulatory_section = f"\n\n{regulatory_context}\n\nWhen making your decision, ensure compliance with the above regulatory guidelines. Reference specific regulations where applicable."

    prompt = f"""You are a Sustainable Credit Officer specializing in agricultural micro-loans.

Analyze the satellite vegetation data AND weather/climate data provided to make a comprehensive loan decision.{regulatory_section}
...
{'- Ensure compliance with LMA Green Lending Guidelines, EU Taxonomy Regulation, and SFDR requirements' if regulatory_context else ''}
{lang_instruction}
...
{f'COMPLIANCE: [Reference specific regulations from the provided context that support your decision]' if regulatory_context else ''}"""
    ...
    try:
        ...
        # Parse the response to extract decision, confidence, reasoning, recommendations, and compliance
        ...
        for line in lines:
            ...
            elif line.upper().startswith('COMPLIANCE:'):
                compliance_text = line.split(':', 1)[1].strip()
                if compliance_text:
                    compliance_citations = [c.strip('- ').strip() for c in compliance_text.split('\n') if c.strip()]
        ...
        if compliance_citations:
            result["compliance_citations"] = compliance_citations
